  ...
  poolPromise
    .then(pool => {
      return pool.request().query(`
        SELECT PostID, PostDescription, PostSong, PostDate, UserName, Playlist, Rating, (SELECT COUNT(*) FROM COMMENTS WHERE COMMENTS.PostID = POSTS.PostID) as numComments
        FROM POSTS 
...
router.route("/:id/:value").put((req, res) => {
  poolPromise
    .then(pool => {
      return pool
        .request()
        .input("PostID", sql.Int, req.params.id)
        .input("Value", sql.Int, req.params.value)
        .query("UPDATE POSTS SET Rating=Rating+@Value WHERE PostID=@PostID");        
    })
    .then(result => {
